...
class SQLModel {
    ...
    async findById(id, columns = []) {
        try {
            const sql = `SELECT ${columns.length > 0 ? columns.join(', ') : '*'} FROM ${this.table} WHERE id = ?`;
    ...
    async findBy(column, value, columns = []) {
        try {
            const sql = `SELECT ${columns.length > 0 ? columns.join(', ') : '*'} FROM ${this.table} WHERE ${column} = ?`;
    ...
    async findAll(columns = []) {
        try {
            const sql = `SELECT ${columns.length > 0 ? columns.join(', ') : '*'} FROM ${this.table}`;
    ...
    async insert(data) {
        try {
            const keys = Object.keys(data).map(key => `\`${key}\``).join(', ');  // Escape column names
    ...
    async insertBulk(dataArray) {
        try {
            ...
            const keys = Object.keys(dataArray[0]).map(key => `\`${key}\``).join(', ');
    ...
    async insertAllowDuplicates(data, duplicateColumn, duplicateValue) {
        try {
            ...
            values.push(duplicateValue); // Add duplicate update value
